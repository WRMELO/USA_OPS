...
# Constantes pre-registradas (nao alterar pos-hoc)
...
def _compute_nelson_we_downside(opw: pd.DataFrame) -> pd.DataFrame:
    rows: list[pd.DataFrame] = []
    for ticker, g in opw.groupby("ticker", sort=False):
        s = g.sort_values("date").copy()
        i_value = pd.to_numeric(s["i_value"], errors="coerce")
        i_ucl = pd.to_numeric(s["i_ucl"], errors="coerce")
        i_lcl = pd.to_numeric(s["i_lcl"], errors="coerce")
        ...
        i_w4_dn = (i_value < i_cl).astype(int).rolling(8, min_periods=8).sum() == 8
        i_w3_dn = (i_value < i_zb_dn).astype(int).rolling(5, min_periods=5).sum() >= 4
        i_w2_dn = (i_value < i_za_dn).astype(int).rolling(3, min_periods=3).sum() >= 2
        i_n3_dn = (i_value.diff() < 0).astype(int).rolling(5, min_periods=5).sum() == 5

        s["nelson_we_dn"] = (i_w4_dn | i_w3_dn | i_w2_dn | i_n3_dn).fillna(False).astype(bool)
        rows.append(s[["date", "ticker", "nelson_we_dn"]])
    if not rows:
        return pd.DataFrame(columns=["date", "ticker", "nelson_we_dn"])
    out = pd.concat(rows, ignore_index=True)
    out = out.sort_values(["ticker", "date"]).reset_index(drop=True)
    return out


def _prepare_holding_timeseries(holdings: pd.DataFrame, market: pd.DataFrame) -> pd.DataFrame:
    rows: list[pd.DataFrame] = []
    for r in holdings.itertuples(index=False):
        h = market[(market["ticker"] == r.ticker) & (market["date"] >= r.entry_date) & (market["date"] <= r.last_date)].copy()
        if h.empty:
            continue
    ...
    out["slope_5d"] = out.groupby("holding_id")["close_operational"].transform(lambda s: (s - s.shift(5)) / s.shift(5))
    out["rank_delta_5d"] = out.groupby("holding_id")["m3_rank"].transform(lambda s: s - s.shift(5))
    return out
...
def _metrics_for_config(
    ...
    for hid, g in ts.groupby("holding_id", sort=False):
        ...
        signal = g[
            ...
            & (g["slope_5d"] <= sl_thr)
            & (g["nelson_we_dn"] == True)
            & (g["rank_delta_5d"] >= rank_delta_thr)
        ]
        if signal.empty:
            cand_hit = False
            cand_date = pd.NaT
    ...
    merged["config_id"] = f"DD{abs(dd_thr):.3f}_SL{abs(sl_thr):.3f}_RD{int(rank_delta_thr)}"
    return merged
...
def _satl_sanity(market: pd.DataFrame) -> dict[str, object]:
    entry_date = pd.Timestamp("2026-05-18")
    entry_price = 9.7982
    target_candidate_date = pd.Timestamp("2026-06-01")
    target_candidate_price = 8.67

    satl = market[(market["ticker"] == "SATL") & (market["date"] >= entry_date)].copy()
    ...
    satl["rank_delta_5d"] = satl["m3_rank"] - satl["m3_rank"].shift(5)
    ...
    for dd in DRAWDOWN_THRESHOLDS:
        for sl in SLOPE_THRESHOLDS:
            for rd in RANK_DELTA_THRESHOLDS:
                cfg_id = f"DD{abs(dd):.3f}_SL{abs(sl):.3f}_RD{int(rd)}"
                sig = satl[
                    (satl["drawdown"] <= dd)
                    & (satl["slope_5d"] <= sl)
                    & (satl["nelson_we_dn"] == True)
                    & (satl["rank_delta_5d"] >= rd)
                ]
                if sig.empty:
                    per_config[cfg_id] = {
                        "cand_hit": False,
...
def main() -> None:
    ...
    opw = pd.read_parquet(
        ...
        columns=["date", "ticker", "close_operational", "i_value", "i_ucl", "i_lcl", "mr_value", "mr_ucl"],
    )
    ...
    nelson = _compute_nelson_we_downside(opw)
    market = opw.merge(scores, on=["date", "ticker"], how="left")
    market = market.merge(nelson, on=["date", "ticker"], how="left")
    market["nelson_we_dn"] = market["nelson_we_dn"].fillna(False).astype(bool)
    ...
    ts = _prepare_holding_timeseries(holdings=holdings, market=market)
    base = _baseline_by_holding(ts=ts)

    results: list[dict[str, object]] = []
    print("=== CALIBRACAO EXIT SIGNAL US V2 ===")
    for dd in DRAWDOWN_THRESHOLDS:
        for sl in SLOPE_THRESHOLDS:
            for rd in RANK_DELTA_THRESHOLDS:
                cfg = _metrics_for_config(ts=ts, base=base, dd_thr=dd, sl_thr=sl, rank_delta_thr=rd)
    ...
    satl_sanity = _satl_sanity(market=market)
    payload = {
        "meta": {
            "task_id": "T-SDC-EXIT-SIGNAL-CALIBRATION-US-V2",
            "constants": {
                "DRAWDOWN_THRESHOLDS": DRAWDOWN_THRESHOLDS,
                "SLOPE_THRESHOLDS": SLOPE_THRESHOLDS,
                "RANK_DELTA_THRESHOLDS": RANK_DELTA_THRESHOLDS,

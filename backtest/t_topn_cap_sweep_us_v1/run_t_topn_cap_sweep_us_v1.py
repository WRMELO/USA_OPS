...
BASELINE_TOP_N = 20
# Limite efetivo do dataset v2: scores M3 iniciam apenas em 2024-10-08.
EFFECTIVE_START = pd.Timestamp("2024-10-08")
# Particao exata da janela util (442 pregoes): 221 + 221.
FIRST_HALF_END = pd.Timestamp("2025-08-26")
SECOND_HALF_START = pd.Timestamp("2025-08-27")
...
IN_CRITERION = (
    ROOT
    / "backtest"
    / "t_topn_cap_sweep_us_v1"
    / "decision_criterion_topn_cap_sweep_us_v1.json"
)
OUT_DIR = ROOT / "backtest" / "t_topn_cap_sweep_us_v1" / "results"
...
def _to_split_v2(day: pd.Timestamp, holdout_end: pd.Timestamp) -> str:
    """Mapeia splits usando a cobertura real do dataset v2.

    Nao reutiliza `_to_split` do modulo t_band_exp_entry_us_v1 porque aquela
    funcao depende de constantes globais do modulo de origem.
    """
...
def main() -> None:
    ...
    curves_df["split"] = curves_df["date"].map(
        lambda d: _to_split_v2(pd.Timestamp(d), holdout_end=holdout_end)
    )
    ...
    obs_path = OUT_DIR / "observations_topn_cap_sweep_us_v1.csv"
    ...
    sanity_path = OUT_DIR / "sanity_gates_report_topn_cap_sweep_us_v1.json"
    ...
    r060_diag_path = OUT_DIR / "r060_gate_diagnostics_topn_cap_sweep_us_v1.json"
    ...
    summary_path = OUT_DIR / "summary_topn_cap_sweep_us_v1.csv"
    ...
    bootstrap_path = OUT_DIR / "bootstrap_stats_topn_cap_sweep_us_v1.json"
    ...
    verdict_path = OUT_DIR / "verdict_topn_cap_sweep_us_v1.json"

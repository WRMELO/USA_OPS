...
IN_DECISION_CRITERION = (
    ROOT / "backtest" / "t088b_spc_enriched_ablation_us_fixed" / "decision_criterion_t088b_us.json"
)

OUT_DIR = ROOT / "backtest" / "t088b_spc_enriched_ablation_us_fixed" / "results"
...
def _build_runs_flags_per_ticker(df_ticker: pd.DataFrame) -> pd.DataFrame:
    ...
    xbar_value = pd.to_numeric(df["xbar_value"], errors="coerce")
    xbar_ucl = pd.to_numeric(df["xbar_ucl"], errors="coerce")
    xbar_lcl = pd.to_numeric(df["xbar_lcl"], errors="coerce")
    xbar_cl = (xbar_ucl + xbar_lcl) / 2.0
    xbar_sigma = (xbar_ucl - xbar_cl) / 3.0
    xbar_za_up = xbar_cl + (2.0 * xbar_sigma)
    xbar_za_dn = xbar_cl - (2.0 * xbar_sigma)
    xbar_zb_up = xbar_cl + xbar_sigma
    xbar_zb_dn = xbar_cl - xbar_sigma

    xbar_above_cl = (xbar_value > xbar_cl).astype(int)
    xbar_below_cl = (xbar_value < xbar_cl).astype(int)
    xbar_above_za = (xbar_value > xbar_za_up).astype(int)
    xbar_below_za = (xbar_value < xbar_za_dn).astype(int)
    xbar_above_zb = (xbar_value > xbar_zb_up).astype(int)
    xbar_below_zb = (xbar_value < xbar_zb_dn).astype(int)

    xbar_w4_up = xbar_above_cl.rolling(8, min_periods=8).sum() == 8
    xbar_w4_dn = xbar_below_cl.rolling(8, min_periods=8).sum() == 8
    xbar_w3_up = xbar_above_zb.rolling(5, min_periods=5).sum() >= 4
    xbar_w3_dn = xbar_below_zb.rolling(5, min_periods=5).sum() >= 4
    xbar_w2_up = xbar_above_za.rolling(3, min_periods=3).sum() >= 2
    xbar_w2_dn = xbar_below_za.rolling(3, min_periods=3).sum() >= 2
    xbar_diff = xbar_value.diff()
    xbar_n3_up = (xbar_diff > 0).rolling(5, min_periods=5).sum() == 5
    xbar_n3_dn = (xbar_diff < 0).rolling(5, min_periods=5).sum() == 5
    runs_xbar = (
        xbar_w4_up
        | xbar_w4_dn
        | xbar_w3_up
    ...
    r_value = pd.to_numeric(df["r_value"], errors="coerce")
    r_ucl = pd.to_numeric(df["r_ucl"], errors="coerce")
    r_bar = r_ucl / D4_N4
    r_sigma = (r_ucl - r_bar) / 3.0
    r_za_up = r_bar + (2.0 * r_sigma)
    r_zb_up = r_bar + r_sigma
    r_above_cl = (r_value > r_bar).astype(int)
    r_above_za = (r_value > r_za_up).astype(int)
    r_above_zb = (r_value > r_zb_up).astype(int)
    ...
    r_diff = r_value.diff()
    r_n3 = (r_diff > 0).rolling(5, min_periods=5).sum() == 5
    ...
    df["runs_xbar"] = runs_xbar.fillna(False).astype(bool)
    df["runs_r"] = runs_r.fillna(False).astype(bool)
    return df
...
def _build_arm_flags_by_day(
    ...
    spc["blocked_b"] = spc["blocked_baseline"] | spc["runs_value"] | spc["runs_disp"]
    spc["blocked_bc"] = spc["blocked_b"] | spc["runs_xbar"] | spc["runs_r"]
...
def main() -> None:
    ...
    obs_df.to_csv(OUT_DIR / "observations_t088b_us.csv", index=False)
    ...
    train_summary.to_csv(OUT_DIR / "summary_TRAIN_t088b_us.csv", index=False)
    holdout_summary.to_csv(OUT_DIR / "summary_HOLDOUT_t088b_us.csv", index=False)
    ...
    phase_stats = {
        "meta": {
            "task_id": "T-088B-SPC-ENRICHED-ABLATION-US-FIXED",
            "cadence": cadence,
            "top_n": top_n,
            ...
            "notes": "Ablation de 3 bracos no US com criterio pre-registrado em decision_criterion_t088b_us.json. Universo filtrado por min_market_cap de winner_us.json para paridade com 09_decide.py.",
        },
    ...
    with (OUT_DIR / "phase_sweep_stats_t088b_us.json").open("w", encoding="utf-8") as fp:
        json.dump(phase_stats, fp, ensure_ascii=False, indent=2)

    print("T-088B-FIXED concluido.")

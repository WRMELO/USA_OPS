"""Backtest T-SDC-BANDEXP-RET62-ENTRY-US-V1.

Estudo read-only pre-registrado:
- Baseline_C4puro: C4 puro sem veto adicional.
- Arm_BandExpRet62: veto na intersecao Flag_BandExp e ret_62>=1.00.
- Arm_BandExp_Isolado: veto apenas por Flag_BandExp (decomposicao descritiva).
- Arm_Ret62_Isolado: veto apenas por ret_62>=1.00 (decomposicao descritiva).
"""
...
PRIMARY_ARM = "Arm_BandExpRet62"
DECOMP_ARMS = ["Arm_BandExp_Isolado", "Arm_Ret62_Isolado"]
...
IN_DECISION_CRITERION = (
    ROOT
    / "backtest"
    / "t_bandexp_ret62_entry_us_v1"
    / "decision_criterion_bandexp_ret62_entry_us_v1.json"
)
OUT_DIR = ROOT / "backtest" / "t_bandexp_ret62_entry_us_v1" / "results"


def _build_ret62_by_day(
    scores_by_day: dict[pd.Timestamp, pd.DataFrame],
) -> dict[pd.Timestamp, set[str]]:
    ret62_by_day: dict[pd.Timestamp, set[str]] = {}
...
def _run_phase_arm(
    ...
    gate_by_day_by_arm: dict[str, dict[pd.Timestamp, set[str]]],
    primary_gate_by_day: dict[pd.Timestamp, set[str]],
    ...
    for reb_idx, d_reb in enumerate(rebalance_days):
        ...
        ret62_set = set(ret62_by_day.get(d_prev, set()))
        vetados_r001 = sorted([t for t in blocked if t in spc_set])
        vetados_ret62 = sorted([t for t in blocked if t in ret62_set])

        overlap_com_par_tier: list[str] = []
        if arm_name in DECOMP_ARMS:
            primary_gate = set(primary_gate_by_day.get(d_prev, set()))
            overlap_com_par_tier = sorted([t for t in blocked if t in primary_gate])
        ...
        observations.append(
            {
                ...
                "overlap_com_par_tier": ";".join(overlap_com_par_tier),
                "n_overlap_com_par_tier": int(len(overlap_com_par_tier)),
...
def _decomposition_summary_rows(
    ...
    for split, summary_df in by_name.items():
        ...
        for arm in DECOMP_ARMS:
            a = prev._subset_means(summary_df, arm)
            ...
            rows.append(
                {
                    ...
                    "delta_sharpe_cost_adj": float(
                        a["mean_sharpe_cost_adj"] - b["mean_sharpe_cost_adj"]
                    ),
                    "mean_veto_rate_arm": mean_veto_rate_arm,
                    "mean_veto_rate_baseline": float(b["mean_veto_rate"]),
                    "teto_veto_ok": bool(
                        prev._is_finite(mean_veto_rate_arm)
                        and mean_veto_rate_arm <= max_veto_rate
                    ),
                }
            )
    return rows
...
def _split_to_dict(rows: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
    ...
    for row in rows:
        ...
        out[split][arm] = {
            ...
            "mean_veto_rate_arm": row["mean_veto_rate_arm"],
            "mean_veto_rate_baseline": row["mean_veto_rate_baseline"],
            "teto_veto_ok": row["teto_veto_ok"],
        }
    return out
...
def main() -> None:
    ...
    ret62_by_day = _build_ret62_by_day(scores_by_day)
    print(f"ret62>=1.00 map carregado para {len(ret62_by_day)} pregoes.")

    all_days = sorted(set(scores_by_day.keys()) | set(flagged_by_day.keys()) | set(ret62_by_day.keys()))
    gate_primary_by_day = {
        d: set(flagged_by_day.get(d, set())) & set(ret62_by_day.get(d, set()))
        for d in all_days
    }
    gate_bandexp_isolado_by_day = {d: set(flagged_by_day.get(d, set())) for d in all_days}
    gate_ret62_isolado_by_day = {d: set(ret62_by_day.get(d, set())) for d in all_days}

    gate_by_day_by_arm: dict[str, dict[pd.Timestamp, set[str]]] = {
        PRIMARY_BASELINE: {},
        PRIMARY_ARM: gate_primary_by_day,
        "Arm_BandExp_Isolado": gate_bandexp_isolado_by_day,
        "Arm_Ret62_Isolado": gate_ret62_isolado_by_day,
    }
    mean_gate_primary = float(np.mean([len(v) for v in gate_primary_by_day.values()])) if gate_primary_by_day else 0.0
    mean_gate_bandexp = (
        float(np.mean([len(v) for v in gate_bandexp_isolado_by_day.values()]))
        if gate_bandexp_isolado_by_day
        else 0.0
    )
    mean_gate_ret62 = (
        float(np.mean([len(v) for v in gate_ret62_isolado_by_day.values()]))
        if gate_ret62_isolado_by_day
        else 0.0
    )
    print(
        "Gate stats por dia: "
        f"primary_mean={mean_gate_primary:.2f} "
        f"bandexp_mean={mean_gate_bandexp:.2f} "
    ...
    for arm_name in ARMS:
        ...
        for phase in range(cadence):
            ...
            phase_obs = _run_phase_arm(
                ...
                gate_by_day_by_arm=gate_by_day_by_arm,
                primary_gate_by_day=gate_primary_by_day,
                spc_blocked_by_day=spc_blocked_by_day,
                ret62_by_day=ret62_by_day,
    ...
    obs_df.to_csv(OUT_DIR / "observations_bandexp_ret62_entry_us_v1.csv", index=False)
    ...
    train_summary.to_csv(OUT_DIR / "summary_TRAIN_bandexp_ret62_entry_us_v1.csv", index=False)
    holdout_summary.to_csv(OUT_DIR / "summary_HOLDOUT_bandexp_ret62_entry_us_v1.csv", index=False)
    sw1_summary.to_csv(OUT_DIR / "summary_SW1_bandexp_ret62_entry_us_v1.csv", index=False)
    sw2_summary.to_csv(OUT_DIR / "summary_SW2_bandexp_ret62_entry_us_v1.csv", index=False)
    ...
    with (OUT_DIR / "bootstrap_stats_bandexp_ret62_entry_us_v1.json").open("w", encoding="utf-8") as fp:
        json.dump(bootstrap_payload, fp, ensure_ascii=False, indent=2)

    max_veto_rate = float(decision_criterion.get("max_veto_rate", 0.35))
    veto_holdout_ok = bool(
        prev._is_finite(arm_holdout["mean_veto_rate"])
        and arm_holdout["mean_veto_rate"] <= max_veto_rate
    )
    veto_sw1_ok = bool(
        prev._is_finite(arm_sw1["mean_veto_rate"])
        and arm_sw1["mean_veto_rate"] <= max_veto_rate
    )
    veto_sw2_ok = bool(
        prev._is_finite(arm_sw2["mean_veto_rate"])
        and arm_sw2["mean_veto_rate"] <= max_veto_rate
    )
    ...
    elif favorecido_pair:
        final_verdict = "FAVORECIDO_BANDEXPRET62"
    else:
        final_verdict = "INCONCLUSIVO"

    decomposition_rows = _decomposition_summary_rows(
    ...
    pd.DataFrame(decomposition_rows).to_csv(
        OUT_DIR / "summary_decomposition_bandexp_ret62_entry_us_v1.csv",
        index=False,
    )
    ...
    verdict_payload = {
        ...
        "decomposition_summary": _split_to_dict(decomposition_rows),
        "checks": checks,
        "thresholds": {
            "max_veto_rate": max_veto_rate,
    ...
    with (OUT_DIR / "verdict_bandexp_ret62_entry_us_v1.json").open("w", encoding="utf-8") as fp:
    ...
    phase_stats = {
        "meta": {
            ...
            "decomposition_arms_no_tier": DECOMP_ARMS,
            "notes": "Materialidade do filtro amplo BandExp intersect ret62>=1.00 no Top-20 de entrada.",
        },
    ...
    with (OUT_DIR / "phase_sweep_stats_bandexp_ret62_entry_us_v1.json").open("w", encoding="utf-8") as fp:

"""Backtest read-only: M3 redundancy reweight US V1.

Task: T-SDC-M3-REDUNDANCY-REWEIGHT-US-V1
"""
...
TASK_ID = "T-SDC-M3-REDUNDANCY-REWEIGHT-US-V1"
IN_BLACKLIST = ROOT / "data" / "ssot" / "blacklist_us.json"
IN_WINNER = ROOT / "config" / "winner_us.json"
IN_CRITERION = (
    ROOT
    / "backtest"
    / "t_m3_redundancy_reweight_us_v1"
    / "decision_criterion_m3_redundancy_reweight_us_v1.json"
)
OUT_DIR = ROOT / "backtest" / "t_m3_redundancy_reweight_us_v1" / "results"

OUT_OBS = OUT_DIR / "observations_m3_redundancy_reweight_us_v1.csv"
OUT_OBS_TICKER = OUT_DIR / "observations_ticker_late_rocket_us_v1.csv"
OUT_CASE_TRACE = OUT_DIR / "case_trace_repl_smwb_us_v1.csv"
OUT_BOOTSTRAP = OUT_DIR / "bootstrap_diagnostics_us_v1.json"
OUT_VERDICT = OUT_DIR / "verdict_m3_redundancy_reweight_us_v1.json"

BASELINE_ARM = "Baseline_C4"
ARMS_FRENTE1 = ["Arm_Dedup1x1", "Arm_DedupVolTilt"]
ARMS_FRENTE2: list[str] = []
...
def main() -> None:
    ...
    case_trace_tickers = [str(x).upper().strip() for x in criterion["runtime_params"].get("case_trace_tickers", ["REPL", "SMWB"])]
    ...
    for d, g in canonical.groupby("date", sort=True):
        ...
        mc_eligible_by_day[d] = set(
            tickers.loc[tickers["market_cap"] >= 300_000_000.0, "ticker"].astype(str).str.upper().str.strip().tolist()
        )

    mc_map = (
        canonical[["date", "ticker", "market_cap"]]
        .dropna(subset=["date", "ticker", "market_cap"])
        .drop_duplicates(subset=["date", "ticker"], keep="last")
        .set_index(["date", "ticker"])["market_cap"]
    )
    ...
    for phase in range(cadence):
        ...
        for reb_idx, d_reb in enumerate(rebalance_days):
            ...
            eligible_300 = mc_eligible_by_day.get(d_prev, set())
            ...
            selected_map: dict[str, list[str]] = {
                BASELINE_ARM: baseline_selected,
                "Arm_Dedup1x1": [],
                "Arm_DedupVolTilt": [],
            }

            dedup_pool = base_pool.copy()
            dedup_pool["score_m3"] = (
                pd.to_numeric(dedup_pool["z_ret"], errors="coerce")
                - pd.to_numeric(dedup_pool["z_vol"], errors="coerce")
            )
            dedup_pool = dedup_pool.dropna(subset=["score_m3"])
            selected_dedup = select_top_n(dedup_pool, top_n=top_n, blacklist=blacklist)
            if selected_dedup:
                selected_map["Arm_Dedup1x1"] = selected_dedup
                substitutes = sorted(set(selected_dedup) - set(baseline_selected))
                n_turnover = int(len(substitutes))
                n_cost = n_turnover
                turnover_rate = float(n_turnover / top_n) if top_n > 0 else float("nan")
                log_ret_arm = _basket_log_return(px_wide, d_prev, d_prev_next, selected_dedup)
                ...
                observations.append(
                    {
                        ...
                        "tickers_baseline": ";".join(baseline_selected),
                        "tickers_arm": ";".join(selected_dedup),
                        "tickers_vetados": "",
            ...
            tilt_pool = base_pool.copy()
            tilt_pool["score_m3"] = (
                pd.to_numeric(tilt_pool["z_ret"], errors="coerce")
                - (1.5 * pd.to_numeric(tilt_pool["z_vol"], errors="coerce"))
            )
            tilt_pool = tilt_pool.dropna(subset=["score_m3"])
            selected_tilt = select_top_n(tilt_pool, top_n=top_n, blacklist=blacklist)
            if selected_tilt:
                selected_map["Arm_DedupVolTilt"] = selected_tilt
                substitutes = sorted(set(selected_tilt) - set(baseline_selected))
                n_turnover = int(len(substitutes))
                n_cost = n_turnover
                turnover_rate = float(n_turnover / top_n) if top_n > 0 else float("nan")
                log_ret_arm = _basket_log_return(px_wide, d_prev, d_prev_next, selected_tilt)
                cost_arm = float(n_cost * 2.0 * friction_one_way_rate / top_n) if top_n > 0 else 0.0
                log_ret_arm_cost_adj = float(log_ret_arm - cost_arm) if _is_finite(log_ret_arm) else float("nan")
                observations.append(
                    {
                        "arm": "Arm_DedupVolTilt",
                        "frente": "score_horizon_dedup",
                        "phase": int(phase),
                        "date": d_reb.date().isoformat(),
                        "d_prev": d_prev.date().isoformat(),
                        ...
                        "turnover_rate": turnover_rate,
                        "tickers_baseline": ";".join(baseline_selected),
                        "tickers_arm": ";".join(selected_tilt),
                        "tickers_vetados": "",
                        "tickers_substitutos": ";".join(substitutes),
                        "log_ret_baseline": log_ret_baseline,
                        "log_ret_arm": log_ret_arm,
            ...
            dedup_rank = (
                dedup_pool["score_m3"].rank(ascending=False, method="min")
                if not dedup_pool.empty
                else pd.Series(dtype=float)
            )
            tilt_rank = (
                tilt_pool["score_m3"].rank(ascending=False, method="min")
                if not tilt_pool.empty
                else pd.Series(dtype=float)
            )

            for ticker in case_trace_tickers:
                t = ticker.upper().strip()
                in_prev = t in prev_scores.index
                in_base_pool = t in base_pool.index
                raw_score_m3 = _safe_float(prev_scores.at[t, "score_m3"]) if in_prev else float("nan")
                raw_m3_rank = _safe_float(prev_scores.at[t, "m3_rank"]) if in_prev else float("nan")
                market_cap = _safe_float(mc_map.get((d_prev, t), float("nan")))
                eligible_300_flag = int(t in eligible_300)
                ret_62_t = _safe_float(prev_scores.at[t, "ret_62"]) if in_prev else float("nan")
                ret10_t = _safe_float(ret10_wide.at[d_prev, t]) if (d_prev in ret10_wide.index and t in ret10_wide.columns) else float("nan")
                score_dedup = _safe_float(dedup_pool.at[t, "score_m3"]) if in_base_pool and t in dedup_pool.index else float("nan")
                score_tilt = _safe_float(tilt_pool.at[t, "score_m3"]) if in_base_pool and t in tilt_pool.index else float("nan")
                rank_dedup = _safe_float(dedup_rank.get(t, float("nan"))) if len(dedup_rank) else float("nan")
                rank_tilt = _safe_float(tilt_rank.get(t, float("nan"))) if len(tilt_rank) else float("nan")
                fwd_ret = _ticker_log_return(px_wide, d_prev, d_prev_next, t)
                ...
                case_trace_rows.append(
                    {
                        ...
                        "eligible_300": eligible_300_flag,
                        "raw_score_m3": raw_score_m3,
                        "raw_m3_rank": raw_m3_rank,
                        "score_m3_dedup": score_dedup,
                        "rank_dedup": rank_dedup,
                        "in_top20_dedup": int(t in selected_map["Arm_Dedup1x1"]),
                        "score_m3_tilt": score_tilt,
                        "rank_tilt": rank_tilt,
                        "in_top20_tilt": int(t in selected_map["Arm_DedupVolTilt"]),
                        "in_top20_baseline": int(t in selected_map[BASELINE_ARM]),
                        "ret_62": ret_62_t,
                        "ret10_d_prev": ret10_t,
                        "forward_logret": fwd_ret,
                    }
                )
    ...
    case_trace_df = pd.DataFrame(case_trace_rows)
    if case_trace_df.empty:
        raise RuntimeError("nenhuma observacao de case_trace gerada")
    case_trace_df = case_trace_df.sort_values(["ticker", "phase", "date"]).reset_index(drop=True)

    OUT_DIR.mkdir(parents=True, exist_ok=True)
    obs_df.to_csv(OUT_OBS, index=False)
    ticker_obs_df.to_csv(OUT_OBS_TICKER, index=False)
    case_trace_df.to_csv(OUT_CASE_TRACE, index=False)

    subsets = ["HOLDOUT", "SW1", "SW2"]
    summary_rows: list[dict[str, Any]] = []
    for subset in subsets:
        sub = _subset_df(obs_df, subset=subset)
        for arm in [BASELINE_ARM] + ARMS_FRENTE1:
            g = sub[sub["arm"] == arm].copy()
            m = _compute_metrics(g)
            gate_value = _nanmean(g["turnover_rate"]) if len(g) else float("nan")
            summary_rows.append(
                {
                    "subset": subset,
                    "arm": arm,
                    ...
                    "gate_metric_name": "turnover_rate",
                }
            )
    ...
    for arm in ARMS_FRENTE1:
        ...
        holdout_gate_series = holdout_arm["turnover_rate"]
        holdout_gate_value = _nanmean(holdout_gate_series)
        holdout_gate_limit = max_turnover_rate
        ...
        arm_bootstrap_payload[arm] = {
            ...
            "holdout_gate_metric_name": "turnover_rate",
    ...
    bootstrap_payload = {
        ...
        "runtime": {
            ...
            "case_trace_tickers": case_trace_tickers,
        },
        "arms_bootstrap_vs_baseline_c4": arm_bootstrap_payload,
    ...
    verdict_payload = {
        ...
        "criterion_file": str(IN_CRITERION.relative_to(ROOT)),
        "fronts": {
            "score_horizon_dedup": {arm: arm_verdict_payload[arm] for arm in ARMS_FRENTE1},
        },
        "late_rocket_diagnostic": late_verdict,
        "notes": [
            "Estudo read-only. Nenhuma mudanca de motor aplicada.",
            "Frente de piso de tamanho nao foi reaberta nesta task.",
    ...
    print(f"wrote {OUT_CASE_TRACE.relative_to(ROOT)}")

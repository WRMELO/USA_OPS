"""Backtest T-LATE-ROCKET-ENTRY-US-V2: gate de entrada por ret_62 + estabilidade.

Estudo read-only, sem alterar motor produtivo.
"""
...
IN_DECISION_CRITERION = (
    ROOT / "backtest" / "t_late_rocket_entry_us_v2" / "decision_criterion_late_rocket_us_v2.json"
)

OUT_DIR = ROOT / "backtest" / "t_late_rocket_entry_us_v2" / "results"

FRICTION_ONE_WAY_RATE = 0.000250  # 2.5 bps one-way
RET62_THRESHOLD = 1.00
ARMS = ["Baseline", "Arm_A_1.00", "Arm_A_1.00_SPC"]
...
SW2_END = pd.Timestamp("2026-03-16")
...
def _to_split_v2(day: pd.Timestamp) -> str:
    if day <= TRAIN_END:
        return "TRAIN"
...
def _arm_gate_set(
    ...
    blocked_ret: set[str] = set()
    if arm_name in {"Arm_A_1.00", "Arm_A_1.00_SPC"}:
        blocked_mask = pd.to_numeric(prev_scores["ret_62"], errors="coerce") >= RET62_THRESHOLD
        blocked_ret = set(prev_scores.loc[blocked_mask].index.astype(str).str.upper().str.strip())

    blocked_spc: set[str] = set()
    if arm_name == "Arm_A_1.00_SPC":
        blocked_spc = set(spc_blocked_by_day.get(d_prev, set()))

    blocked_total = blocked_ret | blocked_spc
    return blocked_total, blocked_spc
...
def _run_phase_arm(
    ...
    for reb_idx, d_reb in enumerate(rebalance_days):
        ...
        blocked_total, blocked_spc = _arm_gate_set(
            arm_name=arm_name,
        ...
        observations.append(
            {
                ...
                "n_veto": n_veto,
                "veto_rate": veto_rate,
                "n_spc_blocked_pool": int(len(blocked_spc)),
...
def _checks_against_baseline(
    ...
    return {
        "arm_better_tail": bool(arm_means["mean_cvar5"] > baseline_means["mean_cvar5"]),
        "arm_better_sharpe": bool(
            arm_means["mean_sharpe_cost_adj"] > baseline_means["mean_sharpe_cost_adj"]
        ),
        "arm_acceptable_veto": bool(arm_means["mean_veto_rate"] <= MAX_VETO_RATE),
    }
...
def main() -> None:
    ...
    obs_df.to_csv(OUT_DIR / "observations_late_rocket_us_v2.csv", index=False)
    ...
    train_summary.to_csv(OUT_DIR / "summary_TRAIN_late_rocket_us_v2.csv", index=False)
    holdout_summary.to_csv(OUT_DIR / "summary_HOLDOUT_late_rocket_us_v2.csv", index=False)
    sw1_summary.to_csv(OUT_DIR / "summary_SW1_late_rocket_us_v2.csv", index=False)
    sw2_summary.to_csv(OUT_DIR / "summary_SW2_late_rocket_us_v2.csv", index=False)
    ...
    for arm in ARMS:
        ...
        veto = means["mean_veto_rate"]
        if not np.isfinite(veto) or veto > MAX_VETO_RATE:
            continue
        cvar = means["mean_cvar5"]
        sharpe = means["mean_sharpe_cost_adj"]
    ...
    if not candidate_arms:
        ...
        best_threshold = None
        best_holdout_means: dict[str, float] | None = None
    else:
        best_arm, _, best_holdout_means = sorted(candidate_arms, key=lambda x: x[1], reverse=True)[0]
        best_threshold = RET62_THRESHOLD

    checks_holdout = _checks_against_baseline(best_holdout_means, baseline_holdout)

    best_sw1_means = sw1_means_by_arm.get(best_arm) if best_arm in sw1_means_by_arm else None
    best_sw2_means = sw2_means_by_arm.get(best_arm) if best_arm in sw2_means_by_arm else None
    checks_sw1 = _checks_against_baseline(best_sw1_means, sw1_means_by_arm["Baseline"])
    checks_sw2 = _checks_against_baseline(best_sw2_means, sw2_means_by_arm["Baseline"])

    stability_check = {
        "SW1": {
            "checks": checks_sw1,
            "baseline_metrics": sw1_means_by_arm["Baseline"],
            "best_arm_metrics": best_sw1_means,
        },
        "SW2": {
            "checks": checks_sw2,
            "baseline_metrics": sw2_means_by_arm["Baseline"],
            "best_arm_metrics": best_sw2_means,
        },
    }

    if best_arm == "nenhum_viavel":
        final_verdict = "INCONCLUSIVO"
    elif (
        checks_holdout["arm_better_tail"]
        and checks_holdout["arm_better_sharpe"]
        and checks_holdout["arm_acceptable_veto"]
    ):
        sw_ok = (
            checks_sw1["arm_better_tail"]
            and checks_sw1["arm_better_sharpe"]
            and checks_sw1["arm_acceptable_veto"]
            and checks_sw2["arm_better_tail"]
            and checks_sw2["arm_better_sharpe"]
            and checks_sw2["arm_acceptable_veto"]
        )
        final_verdict = "CONFIRMA_V2" if sw_ok else "PARCIAL"
    elif (not checks_holdout["arm_better_tail"]) and (not checks_holdout["arm_better_sharpe"]):
        final_verdict = "NAO_CONFIRMA_V2"
    else:
        final_verdict = "INCONCLUSIVO"

    phase_stats = {
        "meta": {
            "task_id": "T-LATE-ROCKET-ENTRY-US-V2",
            "cadence": cadence,
            "top_n": top_n,
            "min_market_cap": min_market_cap,
            "friction_one_way_rate": FRICTION_ONE_WAY_RATE,
            "ret62_threshold": RET62_THRESHOLD,
            ...
            "subwindows": {
                ...
                "SW2": {"start": SW2_START.date().isoformat(), "end": SW2_END.date().isoformat()},
            },
            "arms": ARMS,
            "notes": "Ablation V2 read-only com arm adicional Arm_A_1.00_SPC e checagem de estabilidade por subjanela.",
        },
    ...
    with (OUT_DIR / "phase_sweep_stats_late_rocket_us_v2.json").open("w", encoding="utf-8") as fp:
    ...
    verdict_payload = {
        ...
        "best_arm": best_arm,
        "best_arm_threshold": best_threshold,
        "checks": checks_holdout,
        "stability_check": stability_check,
        "baseline_metrics_holdout": baseline_holdout,
        "best_arm_metrics_holdout": best_holdout_means,
        "max_veto_rate_constraint": MAX_VETO_RATE,
    }
    with (OUT_DIR / "verdict_late_rocket_us_v2.json").open("w", encoding="utf-8") as fp:
        json.dump(verdict_payload, fp, ensure_ascii=False, indent=2)

    print("T-LATE-ROCKET-ENTRY-US-V2 concluido.")

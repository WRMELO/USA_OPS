...
IN_DECISION_CRITERION = (
    ROOT / "backtest" / "t088_spc_enriched_ablation_us" / "decision_criterion_t088_us.json"
)

OUT_DIR = ROOT / "backtest" / "t088_spc_enriched_ablation_us" / "results"
...
def main() -> None:
    ...
    obs_df.to_csv(OUT_DIR / "observations_t088_us.csv", index=False)
    ...
    train_summary.to_csv(OUT_DIR / "summary_TRAIN_t088_us.csv", index=False)
    holdout_summary.to_csv(OUT_DIR / "summary_HOLDOUT_t088_us.csv", index=False)
    ...
    phase_stats = {
        "meta": {
            "task_id": "T-088-SPC-ENRICHED-ABLATION-US",
            "cadence": cadence,
            "top_n": top_n,
            ...
            "notes": "Ablation de 3 bracos no US com criterio pre-registrado em decision_criterion_t088_us.json.",
        },
    ...
    with (OUT_DIR / "phase_sweep_stats_t088_us.json").open("w", encoding="utf-8") as fp:
        json.dump(phase_stats, fp, ensure_ascii=False, indent=2)

    print("T-088-US concluido.")

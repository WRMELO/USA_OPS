...
- feat(autosave+ledger): T-SDC-DRYRUN-US-AUTOLIQUIDACAO-DEFERBUY-RECONSTRUCAO-V1 — corrige `pipeline/painel_diario.py` para gerar `cash_transfers` diarios e separar venda de rebalance (dia D) da compra diferida (D+1+) com plano congelado em `data/daily/pending_rebalance_buy.json`; corrige `pipeline/dryrun_autosave.py` para propagar transferencias calculadas; adiciona guarda prospectiva de `cash_free` negativo em `pipeline/servidor.py::apply_boletim_operations`; inclui script supervisionado de remediacao append-only `scripts/migrate_ledger_20260716_dryrun_rebalance_settlement_buy.py`; amplia cobertura em `tests/test_painel_diario_autosave.py`, `tests/test_dryrun_autosave.py`, `tests/test_servidor_real_boletim.py`; aplica reconstrução de `market_day=2026-07-15` a `2026-07-23` com 36 eventos novos no ledger (`388 -> 424`) e trilha `source=\"remediation\"` em `data/daily/autosave_log.jsonl`. Decision: USA D-163 (ref cruzada: SALA D-166).
...
- feat(live-real+dust): T-SDC-LIVEREAL-FRACIONARIO-LOTE-DEFENSIVA-DUST-US-V1 — apresenta primeiro o layout novo do `/painel` ao Owner (restart 8787/8788), propaga fracionario no contexto real (`pipeline/analise_us.py`), corrige calculo de heat e nao-realizado por lote no `pipeline/real_boletim_web.py`, restringe gatilho SPC defensivo para `INSTAVEL`, valida DFC/Balancete por cheque read-only de `compute_cash` (`CHECK_DFC_CASH_OK`) e corrige dust de venda integral no helper compartilhado (`backtest/run_backtest_variants_us.py`) com evidencia append-only em `backtest/t_sell_all_ticker_fifo_dust_impact_us/results/impact_report_2026-07-23.json`. Inclui cobertura em `tests/test_analise_us.py`, `tests/test_real_boletim_web.py`, `tests/test_run_backtest_variants_us.py`. Decision: SALA D-142 / USA D-159.
- docs(curadoria): T-SDC-LIVEREAL-BOLETIM-CASH-DFC-LAYOUT-US-V1 — fecha a rastreabilidade da correcao do boletim LIVE-REAL US: projeção de rascunho salvo em Balancete/DFC, confirmacao manual de liquidacao pendente via `POST /painel/liquidar`, limiar defensivo alinhado ao GRAVE (`-32,42%`), layout reorganizado com encerramento no ultimo bloco e testes ampliados para ledger, boletim e servidor. Artefatos: `DECISION_LOG.md`, `ROADMAP.md`, `GOVERNANCE.md`, `pipeline/ledger.py`, `pipeline/real_boletim_web.py`, `pipeline/servidor.py`, `tests/test_ledger.py`, `tests/test_real_boletim_web.py`, `tests/test_servidor_real_boletim.py`, `../SALA_DE_CONTROLE/DECISION_LOG.md`, `../SALA_DE_CONTROLE/CHANGELOG.md`, `../SALA_DE_CONTROLE/REGRAS_OPERACIONAIS.md`, `../SALA_DE_CONTROLE/docs/REFERENCIA_FORNO_US_POLITICAS_OPERACIONAIS.md`. Decision: SALA D-141 / USA D-158.
- docs(backtest): T-SDC-R037-CONFIRM-R001-ENTRY-VETO-US-V1 — fecha a rastreabilidade do estudo read-only de veto de entrada por confirmacao (interseccao) entre R-037 severo e R-001 any no mesmo d_prev, comparado com `Arm_R037_Severe` reproduzido como referencia; veredito `R037_CONFIRM_R001ANY=INCONCLUSIVO`. Artefatos: `backtest/t_r037_confirm_r001_entry_veto_us_v1/`, `DECISION_LOG.md`, `ROADMAP.md`, `../SALA_DE_CONTROLE/DECISION_LOG.md`, `../SALA_DE_CONTROLE/CHANGELOG.md`. Decision: SALA D-140 / USA D-157.
